...
public class FloatSampleTools {
	 ...
	 *             if output does not format.getChannels() elements
	 * @see #byte2float(byte[] input, int inByteOffset, Object[] output, int
	 *      outOffset, int frameCount, AudioFormat format, boolean
	 *      allowAddChannel)
	 */
	 ...
	 *            if true, and output has fewer channels than format, then only
	 *            output.length channels are filled
	 * @throws ArrayIndexOutOfBoundsException
	 *             if output does not format.getChannels() elements
	 * @see #byte2float(byte[] input, int inByteOffset, Object[] output, int
	 *      outOffset, int frameCount, AudioFormat format, boolean
	 ...
	 * @param inByteOffset
	 *            index in input where to start the conversion
	 * @param output
	 *            list of float[] arrays which receive the converted audio data.
	 *            if the list does not contain enough elements, or individual
	 *            float arrays are not large enough, they are created.
	 * @param outOffset
	 *            the start offset in <code>output</code>
	 * @param frameCount
	 *            number of frames to be converted
	 * @param format
	 *            the input format. Only packed PCM is allowed
	 * @param allowAddChannel
	 *            if true, channels may be added to <code>output</code> to match
	 *            the number of input channels, otherwise, only the first
	 *            output.size() channels of input data are converted.
	 * @throws IllegalArgumentException
	 ...
	 * 
	 * @param channel
	 *            the channel number to extract from the input audio data
	 * @param input
	 *            the audio data in an byte array
	 * @param inByteOffset
	 *            index in input where to start the conversion
	 * @param output
	 *            the of float array which receives the converted audio data.
	 * @param outOffset
	 *            the start offset in <code>output</code>
	 * @param frameCount
	 *            number of frames to be converted
	 ...
	 *            index in input where to start the conversion
	 * @param output
	 *            the float array that receives the converted audio data
	 * @param outOffset
	 *            the start offset in <code>output</code>
	 * @param frameCount
	 *            number of frames to be converted
	...
	public static void byte2floatInterleaved(byte[] input, int inByteOffset,
		...
		byte2floatGeneric(input, inByteOffset,
				format.getFrameSize() / format.getChannels(), output,
				outOffset, frameCount * format.getChannels(), format);
	}
	 ...
	 * 
	 * @param sampleCount
	 *            number of samples to be written to output
	 * @param inByteStep
	 ...
	 * the number of elements in <code>input</code>. If <code>input</code> does
	 * not provide enough channels, an </code>IllegalArgumentException<code>
	 * is thrown.
	 * <p>
	 ...
	 * 
	 * @param input
	 *            a List of float arrays with the input audio data
	 * @param inOffset
	 *            index in the input arrays where to start the conversion
	 * @param output
	 *            the byte array that receives the converted audio data
	 * @param outByteOffset
	 *            the start offset in <code>output</code>
	 * @param frameCount
	 *            number of frames to be converted.
	 * @param format
	 *            the output format. Only packed PCM is allowed
	 * @param ditherBits
	 ...
	 *            an array of float[] arrays
	 * @param channels
	 *            how many channels to use from the input array
	 * @param frameSize
	 *            only as optimization, the number of bytes per sample frame
	 * @throws ArrayIndexOutOfBoundsException
	 *             if one of the parameters is out of bounds
	 * @see #float2byte(Object[], int, byte[], int, int, AudioFormat, float)
	 */
	 ...
	 * <code>input[inOffset + (frameCount * format.getChannels()) - 1]</code> to
	 * byte values from <code>output[outByteOffset]</code> to
	 * <code>output[outByteOffset + (frameCount * format.getFrameSize()) - 1]</code>
	 * <p>
	 * Dithering should be used when the output resolution is significantly
	 * lower than the original resolution. This includes if the original data
	 ...
	 * 
	 * @param input
	 *            the audio data in normalized samples
	 * @param inOffset
	 *            index in input where to start the conversion
	 * @param output
	 *            the byte array that receives the converted audio data
	 ...
	 * 
	 * @param sampleCount
	 *            number of samples in input to be converted.
	 * @param outByteStep
	 *            how many bytes advance for each input sample in
	 *            <code>input</code>.
	 * @throws IllegalArgumentException
	 *             if one of the parameters is out of bounds
	 * @see #float2byteInterleaved(float[],int,byte[],int,int,AudioFormat,float)
